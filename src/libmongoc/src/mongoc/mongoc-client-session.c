   ...
   bson_error_t *error)
{
   ...
   while (true) {
      ...
      res = cb (session, ctx, &cb_reply, error);
      ...
      if (!res) {
         ...
         if (mongoc_error_has_label (cb_reply, TRANSIENT_TXN_ERR) &&
             !timeout_exceeded (expire_at)) {
            bson_destroy (cb_reply);
            cb_reply = NULL;
            continue;
         }
    ...
    access it through cb_reply so cleanup in DONE is simpler. */
      bson_destroy (cb_reply);
      cb_reply = &local_reply;
      ...
      while (true) {
         res =
            mongoc_client_session_commit_transaction (session, cb_reply, error);

         if (!res) {
            if (mongoc_error_has_label (cb_reply, UNKNOWN_COMMIT_RESULT) &&
                !timeout_exceeded (expire_at)) {
            ...
            if (mongoc_error_has_label (cb_reply, TRANSIENT_TXN_ERR) &&
               ...
               /* In the case of a transient txn error, go back to outside loop.
                  We must set the cb_reply to NULL so it may be used by the cb.
                  */
               bson_destroy (cb_reply);
               cb_reply = NULL;
               break;
   ...
   /* At this point, cb_reply is either pointing to the user's reply
